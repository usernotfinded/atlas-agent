...
SCRIPT = ROOT / "scripts" / "check_v068_release_prep.py"
...
def _load_script_module() -> ModuleType:
    ...
    sys.modules["check_v068_release_prep"] = mod
...
class TestPlanningModeValid:
    def test_planning_mode_fails_after_bump(self) -> None:
        """Planning mode fails on real repo because source is now 0.6.8."""
        result = _run_script()
    ...
    def test_json_has_required_keys(self) -> None:
        result = _run_script("--json")
        data = json.loads(result.stdout)
        assert data["artifact_type"] == "v068_release_prep_report"
    ...
    def test_release_notes_exist_in_planning_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        fake_notes = tmp_path / "v0.6.8.md"
        fake_notes.write_text("# v0.6.8\n")
    ...
    def test_changelog_has_release_entry_in_planning_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            code, result = mod.run_check()
            assert code == 1
            assert any("must not contain [0.6.8]" in e for e in result["errors"])
        finally:
            mod.CHANGELOG = original

    def test_unselected_candidate_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        fake_json = tmp_path / "v0.6.8-candidates.json"
        fake_json.write_text(
            json.dumps({
                "artifact_type": "v068_patch_candidate_inventory",
                "schema_version": 1,
                "release": "v0.6.8",
                "candidates": [
                    {
                        "id": "CAND-010",
                        "selected_for_v068": True,
                        "implemented": False,
                    }
                ],
                "rejected": [],
    ...
    def test_v067_history_missing_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        original_notes = mod.V067_RELEASE_NOTES
        original_status = mod.V067_TRUST_STATUS
        try:
            mod.V067_RELEASE_NOTES = tmp_path / "missing-v067.md"
            mod.V067_TRUST_STATUS = tmp_path / "missing-v067-status.md"
            code, result = mod.run_check()
            assert code == 1
            assert any("v0.6.7" in e for e in result["errors"])
        finally:
            mod.V067_RELEASE_NOTES = original_notes
            mod.V067_TRUST_STATUS = original_status


class TestReleasePrepMode:
    def test_release_prep_mode_passes_on_real_repo(self) -> None:
        """Release-prep mode passes on real repo because v0.6.8 artifacts are present."""
    ...
    def test_release_prep_readme_stale_version_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            fake_readme.write_text(
                "# README\n\n> **Current Status (v0.6.7)** — package/source version is `0.6.7`;\n"
            )
            mod.README = fake_readme
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("package/source version is 0.6.8" in e for e in result["errors"])
        finally:
    ...
    def test_release_prep_security_stale_version_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            fake_security.write_text(
                "# Security\n\n| 0.6.7 (main) | Yes — active development |\n"
            )
            mod.SECURITY = fake_security
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("0.6.8 (main)" in e for e in result["errors"])
        finally:
    ...
    def test_release_prep_trust_readme_stale_version_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("source package version on main is 0.6.8" in e for e in result["errors"])
        finally:
            mod.TRUST_README = original_trust_readme

    def test_release_prep_missing_demo_proof_checker_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        original = mod.DEMO_PROOF_CHECKER
        try:
            mod.DEMO_PROOF_CHECKER = tmp_path / "nonexistent.py"
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("Demo proof checker missing" in e for e in result["errors"])
        finally:
            mod.DEMO_PROOF_CHECKER = original

    def test_release_prep_missing_demo_command_smoke_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        original = mod.DEMO_COMMAND_SMOKE
        try:
            mod.DEMO_COMMAND_SMOKE = tmp_path / "nonexistent.py"
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("Demo command smoke checker missing" in e for e in result["errors"])
        finally:
            mod.DEMO_COMMAND_SMOKE = original

    def test_release_prep_demo_not_referenced_in_notes_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        original = mod.RELEASE_NOTES
        try:
            fake_notes = tmp_path / "v0.6.8.md"
            fake_notes.write_text("# v0.6.8\n\nSome release notes without demo references.\n")
            mod.RELEASE_NOTES = fake_notes
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("Release notes do not reference check_demo_proof" in e for e in result["errors"])
            assert any("Release notes do not reference check_demo_command_smoke" in e for e in result["errors"])
        finally:
            mod.RELEASE_NOTES = original
