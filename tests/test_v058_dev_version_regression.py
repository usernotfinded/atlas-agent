"""Regression tests for v0.5.8rc1 transition.

Verifies the correct lifecycle model:
- current main = 0.5.8rc1
...
def test_pyproject_version_is_current_dev() -> None:
    ...
    with open(ROOT / "pyproject.toml", "rb") as f:
        data = tomllib.load(f)
    assert data.get("project", {}).get("version") == "0.5.8rc1"
...
def test_version_consistency_script_accepts_dev() -> None:
    ...
    assert "0.5.8rc1" in result.stdout
    assert "v0.5.7" in result.stdout
...
def test_release_checklist_can_mention_historical_stable() -> None:
    ...
    assert "0.5.8rc1" in text
    # Can also mention historical stable
    assert "v0.5.7" in text or "0.5.7" in text
