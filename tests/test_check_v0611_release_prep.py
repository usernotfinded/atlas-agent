# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    tests/test_check_v0611_release_prep.py
# PURPOSE: Verifies check v0611 release prep behavior and regression
#         expectations.
# DEPS:    importlib, json, subprocess, sys, pathlib, types.
# ==============================================================================

"""Tests for v0.6.11 release prep checker.
...
SCRIPT = ROOT / "scripts" / "check_v0611_release_prep.py"
...
def _load_script_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location("check_v0611_release_prep", SCRIPT)
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules["check_v0611_release_prep"] = mod
...
class TestPlanningModeValid:
    def test_planning_mode_fails_after_bump(self) -> None:
        """Planning mode fails on real repo because source is now 0.6.11."""
        result = _run_script()
    ...
    def test_json_has_required_keys(self) -> None:
        result = _run_script("--json")
        data = json.loads(result.stdout)
        assert data["artifact_type"] == "v0611_release_prep_report"
    ...
    def test_missing_planning_version_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            assert code == 1
            assert any("0.6.10" in e for e in result["errors"])
        finally:
    ...
    def test_v0610_history_missing_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        original_notes = mod.V0610_RELEASE_NOTES
        original_status = mod.V0610_TRUST_STATUS
        try:
            mod.V0610_RELEASE_NOTES = tmp_path / "missing-v0610.md"
            mod.V0610_TRUST_STATUS = tmp_path / "missing-v0610-status.md"
            code, result = mod.run_check()
            assert code == 1
            assert any("v0.6.10" in e for e in result["errors"])
        finally:
            mod.V0610_RELEASE_NOTES = original_notes
            mod.V0610_TRUST_STATUS = original_status


class TestReleasePrepMode:
    def test_release_prep_mode_fails_on_real_repo_after_cutover(self) -> None:
        """Release-prep mode fails on real repo after cutover because v0.6.11 is now public."""
    ...
    def test_release_prep_premature_tag_claim_fails(self, tmp_path: Path) -> None:
        """A doc that mentions v0.6.11 and claims the tag was created must fail."""
        mod = _load_script_module()
        fake_notes = tmp_path / "v0.6.11.md"
        fake_notes.write_text(
            "# v0.6.11\n\nThe v0.6.11 tag created and the GitHub release created.\n"
        )
        original = mod.RELEASE_NOTES
        try:
            mod.RELEASE_NOTES = fake_notes
    ...
    def test_release_prep_premature_public_claim_fails(self, tmp_path: Path) -> None:
        ...
        fake_notes = tmp_path / "v0.6.11.md"
        fake_notes.write_text(
            "# v0.6.11\n\nv0.6.11 is the current public release.\n"
        )
        original = mod.RELEASE_NOTES
        try:
            mod.RELEASE_NOTES = fake_notes
    ...
    def test_release_prep_readme_stale_version_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            fake_readme.write_text(
                "# README\n\n> **Current Status (v0.6.10)** — package/source version is `0.6.10`;\n"
            )
            mod.README = fake_readme
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("package/source version is 0.6.11" in e for e in result["errors"])
        finally:
    ...
    def test_release_prep_security_stale_version_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            fake_security.write_text(
                "# Security\n\n| 0.6.10 (main) | Yes — active development |\n"
            )
            mod.SECURITY = fake_security
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("0.6.11 (main)" in e for e in result["errors"])
        finally:
    ...
    def test_release_prep_trust_readme_stale_version_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("source package version on main is 0.6.11" in e for e in result["errors"])
        finally:
    ...
    def test_release_prep_metadata_prepared_state_fails_if_public(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            code, result = mod.run_check(release_prep=True)
            assert code == 1
            assert any("v0.6.11 status must be 'prepared'" in e for e in result["errors"])
...
class TestPostReleaseMode:
    def test_post_release_mode_fails_on_real_repo_after_later_cutover(self) -> None:
        """Post-release mode fails on real repo after v0.6.12 cutover because v0.6.11 is historical."""
        result = _run_script("--post-release")
        assert result.returncode == 1, result.stdout + result.stderr
        assert "FAIL" in result.stdout
        assert "post-release" in result.stdout

    def test_post_release_json_fails_on_real_repo_after_later_cutover(self) -> None:
    ...
    def test_post_release_missing_public_tag_record_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            assert code == 1
            assert any("Release metadata missing v0.6.11 record" in e for e in result["errors"])
        finally:
            mod.RELEASE_METADATA = original

    def test_post_release_v0611_not_current_public_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        fake_metadata = tmp_path / "release-metadata.json"
        fake_metadata.write_text(
            json.dumps({
    ...
    def test_post_release_rejects_stale_current_public_claim(
        ...
        try:
            ...
            assert code == 1
            assert any(
                "current_public_release mismatch: expected v0.6.11" in error
    ...
    def test_post_release_rejects_stale_next_planned_claim(
        ...
        try:
            ...
            assert code == 1
            assert any(
                "next_planned_release mismatch: expected v0.6.12" in error
                for error in result["errors"]
            )
        finally:
            mod.RELEASE_METADATA = original
    ...
    def test_post_release_rejects_pypi_published(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            assert any(
                "v0.6.11 pypi_published must be false" in error
                for error in result["errors"]
            )
        finally:
            mod.RELEASE_METADATA = original
