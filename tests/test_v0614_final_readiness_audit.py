...
def _copy_minimal_repo(tmp_path: Path) -> Path:
    ...
    for rel in [
        ...
        "docs/releases/v0.6.14-final-readiness-audit.json",
...
def test_checker_json_parses():
    ...
    assert data["artifact_type"] == "v0614_final_readiness_audit_check"
    assert data["valid"] is True
    assert data["errors"] == []


def test_checker_fails_if_audit_json_omits_candidate(tmp_path):
    repo = _copy_minimal_repo(tmp_path)
    _mutate_json(
        repo,
        lambda data: data.update(
            {"candidates": [c for c in data["candidates"] if c["id"] != "CAND-007"]}
        ),
    )
    ...
    assert "CAND-007" in result.stdout


def test_checker_fails_if_source_version_is_wrong(tmp_path):
    repo = _copy_minimal_repo(tmp_path)
    _mutate_json(repo, lambda data: data.update({"source_version": "0.6.14"}))
...
def test_checker_fails_if_audit_claims_v0614_released(tmp_path):
    repo = _copy_minimal_repo(tmp_path)
    _append_markdown(repo, "v0.6.14 is released.")
...
def test_checker_fails_if_audit_claims_tag_or_release_created(tmp_path):
    repo = _copy_minimal_repo(tmp_path)
    _mutate_json(
        repo,
...
def test_checker_fails_if_audit_implies_owner_authorization(tmp_path):
    repo = _copy_minimal_repo(tmp_path)
...
def test_checker_fails_on_live_ready_wording(tmp_path):
    repo = _copy_minimal_repo(tmp_path)
...
def test_checker_fails_on_guaranteed_profit_wording(tmp_path):
    repo = _copy_minimal_repo(tmp_path)
...
def test_checker_fails_if_required_evidence_checker_missing_from_gates(tmp_path):
    ...
    for rel in [
        ...
        path.write_text(
            text.replace(
                ...
                "scripts/removed_v0614_paper_portfolio_evidence.py",
            ),
            encoding="utf-8",
        )
    ...
    result = _run_temp(repo)
    assert result.returncode == 1
    assert "check_v0614_paper_portfolio_evidence.py" in result.stdout
