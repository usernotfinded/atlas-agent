...
def mock_env(monkeypatch):
    ...
    def mock_read_text(self, *args, **kwargs):
        ...
        if name == "pyproject.toml":
            return 'version = "0.5.9.4"'
        if name == "src/atlas_agent/__init__.py":
            return '__version__ = "0.5.9.4"'
        ...
        if name == "CHANGELOG.md":
            return "[0.5.9.4]"
        if name == "README.md":
            return "Current Status (v0.5.9.4)"
        if name == "SECURITY.md":
            return "v0.5.9.4"
        return original_read_text(self, *args, **kwargs)
    ...
    def mock_run_cmd(cmd, check=True, cwd=None):
        ...
        if "update check --dry-run" in cmd:
            return "Current version: 0.5.9.4", 0, ""
        if "from atlas_agent.update.sources" in cmd:
            if "v0.5.9.4.dev0" in cmd:
                return "False", 0, ""
            return "True", 0, ""
...
def test_release_assurance_valid(mock_env, tmp_path, monkeypatch):
    monkeypatch.setattr("sys.argv", ["release_assurance.py", "--version", "v0.5.9.4", "--output", str(tmp_path)])
