...
from omni_trade_ai.scheduler.github_actions import (
    SCHEDULED_ROUTINES,
...
def test_github_actions_workflow_file_is_generated(tmp_path) -> None:
    ...
    assert path.exists()
    assert path == tmp_path / ".github" / "workflows" / "omni-trade-routines.yml"
...
def test_github_actions_workflow_contains_all_five_routines(tmp_path) -> None:
    ...
    for routine in SCHEDULED_ROUTINES:
        assert f"omni-trade routine run {routine} --mode paper" in text
