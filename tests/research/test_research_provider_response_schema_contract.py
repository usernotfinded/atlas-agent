...
from atlas_agent.research.provider_response_schema_contract import (
    PROVIDER_RESPONSE_SCHEMA_CONTRACT_VERSION,
    _BOOLEAN_SAFETY_FLAGS,
    build_provider_response_schema_contract_dict,
    create_provider_response_schema_contract,
    find_provider_response_schema_contract_by_id,
    iter_provider_response_schema_contract_artifacts,
    load_and_validate_provider_response_schema_contract,
    provider_response_schema_contract_sha256,
    replay_provider_response_schema_contract,
    safe_validate_provider_response_schema_contract_data,
    summarize_provider_response_schema_contract_state,
)
...
def _full_chain_to_pairing(tmp_path: Path, monkeypatch) -> tuple[str, str, str, str]:
    ...
    return run_id, preview_id, intake_policy_id, pairing_id
...
class TestProviderResponseSchemaContractConfigless:
    def test_schema_contract_does_not_load_config_or_secrets(self, tmp_path: Path, monkeypatch, capsys) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        ...
        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            ...
            code = main(["research", "provider-response-schema-contract", pairing_id, "--json"])

        out = capsys.readouterr().out
        data = json.loads(out)
        assert code == 0
        assert data["ok"] is True
        assert data["status"] == "research_provider_response_schema_contract_created"
        assert "provider_response_schema_contract_id" in data
    ...
    def test_schema_contract_list_does_not_load_config(self, tmp_path: Path, monkeypatch, capsys) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)

        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            "atlas_agent.config.secrets.load_atlas_secrets", side_effect=_raise_if_called
        ):
            code = main(["research", "provider-response-schema-contract-list", "--json"])

        out = capsys.readouterr().out
        data = json.loads(out)
        assert code == 0
        assert data["ok"] is True
        assert data["status"] == "research_provider_response_schema_contract_list"
        assert len(data["items"]) >= 1

    def test_schema_contract_show_does_not_load_config(self, tmp_path: Path, monkeypatch, capsys) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        result = create_provider_response_schema_contract(tmp_path, pairing_id)
        contract_id = result["provider_response_schema_contract_id"]

        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            "atlas_agent.config.secrets.load_atlas_secrets", side_effect=_raise_if_called
        ):
            code = main(["research", "provider-response-schema-contract-show", contract_id, "--json"])

        out = capsys.readouterr().out
        data = json.loads(out)
        assert code == 0
        assert data["ok"] is True
        assert data["status"] == "research_provider_response_schema_contract_shown"
        assert data["provider_response_schema_contract_id"] == contract_id

    def test_schema_contract_validate_does_not_load_config(self, tmp_path: Path, monkeypatch, capsys) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        result = create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            ...
            code = main(["research", "provider-response-schema-contract-validate", contract_id, "--json"])
    ...
    def test_schema_contract_replay_does_not_load_config(self, tmp_path: Path, monkeypatch, capsys) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        result = create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            ...
            code = main(["research", "provider-response-schema-contract-replay", contract_id, "--json"])
    ...
    def test_schema_contract_summary_does_not_load_config(self, tmp_path: Path, monkeypatch, capsys) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            ...
            code = main(["research", "provider-response-schema-contract-summary", run_id, "--json"])
    ...
    def test_schema_contract_doctor_does_not_load_config(self, tmp_path: Path, monkeypatch, capsys) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            ...
            code = main(["research", "provider-response-schema-contract-doctor", run_id, "--json"])

        out = capsys.readouterr().out
        data = json.loads(out)
        assert code == 0
        assert data["ok"] is True
        assert data["status"] == "research_provider_response_schema_contract_doctor"
        assert data["manual_review_gate_open"] is False
        assert data["future_response_artifact_present"] is False
        assert data["provider_response_trusted"] is False


class TestProviderResponseSchemaContractCreation:
    def test_valid_schema_contract_creates_artifact(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        result = create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        assert result["status"] == "research_provider_response_schema_contract_created"
        ...
        assert data["artifact_type"] == "provider_response_schema_contract"
        assert data["contract_version"] == PROVIDER_RESPONSE_SCHEMA_CONTRACT_VERSION
        assert data["schema_version"] == RESEARCH_ARTIFACT_SCHEMA_VERSION
        assert "expected_response_shape" in data
        assert "allowed_response_fields" in data
        assert "rejected_response_fields" in data
        assert "schema_validation_policy" in data
        assert "unsafe_content_policy" in data
        assert "manual_review_gate_policy" in data
        ...
        assert "review_result_policy" in data
        assert "future_response_artifact_requirements" in data
        assert data["manual_review_gate_open"] is False
        assert data["automatic_review_allowed"] is False
        assert data["future_response_artifact_present"] is False
        assert data["provider_response_received"] is False
        assert data["provider_response_trusted"] is False
        assert data["provider_response_can_create_orders"] is False
        assert data["response_schema_allows_trading_signal"] is False
        assert data["response_schema_allows_broker_call"] is False
    ...
    def test_artifact_path_is_correct(self, tmp_path: Path, monkeypatch) -> None:
        ...
        assert "provider_response_schema_contracts" in result["artifact_path"]
    ...
    def test_output_denylist_clean(self, tmp_path: Path, monkeypatch, capsys) -> None:
        ...
        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            ...
            main(["research", "provider-response-schema-contract-list", "--json"])
...
class TestProviderResponseSchemaContractArtifactDenylist:
...
class TestProviderResponseSchemaContractSummary:
    def test_summary_on_valid_run_reports_schema_contract_recorded(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)

        result = summarize_provider_response_schema_contract_state(tmp_path, run_id)
        assert result["ok"] is True
        assert result["status"] == "research_provider_response_schema_contract_summary"
        assert result["response_schema_status"] == "response_schema_contract_recorded"
        assert result["manual_review_gate_open"] is False
        assert result["future_response_artifact_present"] is False
        assert result["provider_response_trusted"] is False

    def test_summary_before_schema_contract_exists_reports_safe_missing(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        ...
        result = summarize_provider_response_schema_contract_state(tmp_path, run_id)
        assert result["ok"] is True
        assert result["status"] == "missing_provider_response_schema_contract"
        assert result["provider_response_schema_contract_id"] is None
        assert result["manual_review_gate_open"] is False
        assert result["future_response_artifact_present"] is False
    ...
    def test_summary_on_missing_run_fails_safely(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        result = summarize_provider_response_schema_contract_state(tmp_path, "run-99999999")
        assert result["ok"] is True
        assert result["status"] == "missing_provider_response_schema_contract"
    ...
    def test_summary_does_not_write_artifacts_or_events(self, tmp_path: Path, monkeypatch) -> None:
        ...
        contracts_before = list((tmp_path / ".atlas" / "research").rglob("provider_response_schema_contracts/*.json"))
        count_before = len(contracts_before)

        summarize_provider_response_schema_contract_state(tmp_path, run_id)

        events_after = list((tmp_path / "events").glob("*.jsonl"))
        contracts_after = list((tmp_path / ".atlas" / "research").rglob("provider_response_schema_contracts/*.json"))
        assert len(contracts_after) == count_before
        assert len(events_after) == len(events_before)


class TestProviderResponseSchemaContractDoctor:
    def test_doctor_on_valid_current_run_reports_safe_state(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        result = doctor_provider_response_schema_contract(tmp_path, run_id)
        assert result["ok"] is True
        assert result["status"] == "research_provider_response_schema_contract_doctor"
        assert result["schema_health"] in ("schema_recorded_no_response_present", "incomplete_expected", "blocked_until_response_artifact_exists")
        assert result["manual_review_gate_open"] is False
        assert result["future_response_artifact_present"] is False
        assert result["provider_response_trusted"] is False
        assert "future_provider_response_artifact" in result["missing_artifacts"]

    def test_doctor_reports_manual_review_gate_required(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)
    ...
    def test_doctor_before_schema_contract_exists_reports_safe_missing(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        ...
        assert result["ok"] is True
        assert result["status"] == "research_provider_response_schema_contract_doctor"
        assert result["schema_health"] == "schema_contract_missing"

    def test_doctor_on_missing_run_fails_safely(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        result = summarize_provider_response_schema_contract_state(tmp_path, "run-99999999")
        assert result["ok"] is True
    ...
    def test_doctor_does_not_write_artifacts_or_events(self, tmp_path: Path, monkeypatch) -> None:
        ...
        from atlas_agent.research.provider_response_schema_contract import doctor_provider_response_schema_contract
        doctor_provider_response_schema_contract(tmp_path, run_id)
    ...
    def test_doctor_output_denylist_clean(self, tmp_path: Path, monkeypatch, capsys) -> None:
        ...
        with patch("atlas_agent.cli.AtlasConfig.from_env", side_effect=_raise_if_called), patch(
            ...
            main(["research", "provider-response-schema-contract-doctor", run_id, "--json"])
...
class TestProviderResponseSchemaContractSafetyTamper:
    def test_tampered_lineage_fails_closed(self, tmp_path: Path, monkeypatch) -> None:
        ...
        data["source_provider_request_response_pairing_id"] = "tampered-id"
        artifact_path.write_text(json.dumps(data), encoding="utf-8")

        cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
    ...
    def test_forbidden_positive_claims_detected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        for value in unsafe_values:
            data["response_schema_status"] = value
            cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
            assert error is not None, f"Expected error for status={value}"
            assert cleaned is None
            data["response_schema_status"] = "response_schema_contract_recorded"


class TestProviderResponseSchemaContractReplayValidation:
    def test_valid_schema_contract_validates(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        result = create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        artifact = load_and_validate_provider_response_schema_contract(
            tmp_path / result["artifact_path"], tmp_path
        )
        assert artifact["provider_response_schema_contract_id"] == contract_id
    ...
    def test_hash_mismatch_detected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
        assert error == "provider_response_schema_contract_hash_mismatch"
        assert cleaned is None
    ...
    def test_source_pairing_missing_detected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        data["artifact_hash"] = provider_response_schema_contract_sha256(data)
        artifact_path.write_text(json.dumps(data), encoding="utf-8")

        cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
        assert error == "provider_response_schema_contract_source_pairing_missing"
        assert cleaned is None
    ...
    def test_source_pairing_hash_mismatch_detected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
        assert error == "provider_response_schema_contract_source_pairing_hash_mismatch"
        assert cleaned is None
    ...
    def test_source_response_intake_missing_detected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
        assert error == "provider_response_schema_contract_source_response_intake_missing"
        assert cleaned is None
    ...
    def test_source_response_intake_hash_mismatch_detected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
        assert error == "provider_response_schema_contract_source_response_intake_hash_mismatch"
        assert cleaned is None
    ...
    def test_source_payload_preview_missing_detected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
        assert error == "provider_response_schema_contract_source_payload_preview_missing"
        assert cleaned is None
    ...
    def test_source_payload_preview_hash_mismatch_detected(self, tmp_path: Path, monkeypatch) -> None:
        ...
        cleaned, error = safe_validate_provider_response_schema_contract_data(data, workspace_path=tmp_path)
        assert error == "provider_response_schema_contract_source_payload_preview_hash_mismatch"
        assert cleaned is None
    ...
    def test_replay_mismatch_envelope_works(self, tmp_path: Path, monkeypatch) -> None:
        ...
        replay = replay_provider_response_schema_contract(tmp_path, contract_id)
        assert replay["ok"] is True
        assert replay["match"] is True


class TestProviderResponseSchemaContractTimelineCheckDossier:
    def test_check_artifacts_counts_schema_contracts(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        assert result["counts"]["provider_response_schema_contracts"] >= 1

    def test_check_artifacts_detects_schema_contract_tampering(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        result = create_provider_response_schema_contract(tmp_path, pairing_id)
        artifact_path = tmp_path / result["artifact_path"]
        data = json.loads(artifact_path.read_text(encoding="utf-8"))
        data["manual_review_gate_open"] = True
        data["artifact_hash"] = provider_response_schema_contract_sha256(data)
        artifact_path.write_text(json.dumps(data), encoding="utf-8")
        ...
        assert "provider_response_schema_contract_impossible_boolean" in issue_codes

    def test_timeline_links_schema_contract(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)
                                                    ...
                                                    for pp in b.get("provider_outbound_payload_previews", []):
                                                        for intake in pp.get("provider_response_intake_policies", []):
                                                            for pairing in intake.get("provider_request_response_pairings", []):
                                                                if pairing.get("provider_response_schema_contracts"):
                                                                    found = True
        assert found, "Timeline did not link schema contract under pairing"

    def test_dossier_includes_schema_contract_summary(self, tmp_path: Path, monkeypatch) -> None:
        _ensure_workspace(tmp_path)
        monkeypatch.chdir(tmp_path)
        run_id, preview_id, intake_policy_id, pairing_id = _full_chain_to_pairing(tmp_path, monkeypatch)
        create_provider_response_schema_contract(tmp_path, pairing_id)
        ...
        assert result["artifact_counts"]["provider_response_schema_contracts"] >= 1
        assert "provider_response_schema_contract" in result["summaries"]
        assert result["summaries"]["provider_response_schema_contract"]["contract_count"] >= 1
        assert result["summaries"]["provider_response_schema_contract"]["manual_review_gate_open"] is False
        assert result["summaries"]["provider_response_schema_contract"]["future_response_artifact_present"] is False


class TestProviderResponseSchemaContractInvalidLeakage:
    """Regression tests: invalid/tampered schema contract artifacts must not leak raw fields through list/timeline/check-artifacts."""
    ...
    def test_tampered_contract_id_does_not_leak_through_list(self, tmp_path: Path, monkeypatch, capsys) -> None:
        def tamper(data):
            data["provider_response_schema_contract_id"] = "APCA_SECRET_TOKEN_sk-LEAKEDSECRET_broker.example.com"
            data["artifact_hash"] = provider_response_schema_contract_sha256(data)
        ...
        assert invalid_items[0]["provider_response_schema_contract_id"] == "<invalid>"
        assert invalid_items[0]["provider_id"] == "unknown"
        assert invalid_items[0]["model_id"] == "unknown"
        assert invalid_items[0]["response_schema_status"] == "invalid"
        assert invalid_items[0]["response_schema_state"] == "invalid"
        assert invalid_items[0].get("error_code") == "invalid_provider_response_schema_contract_artifact"
...
class TestTimelineCircularReference:
    def test_timeline_after_full_chain_is_valid_json_and_denylist_clean(self, tmp_path: Path, monkeypatch) -> None:
        """Regression: timeline must not contain circular references after schema contract is created."""
        import json
        from atlas_agent.research.session import build_research_timeline
        ...
        # Must include the schema contract nested under pairing
                                                        ...
                                                        for ip in pr.get("provider_response_intake_policies", []):
                                                            ...
                                                            for pairing in pairings:
                                                                ...
                                                                for contract in contracts:
                                                                    if contract.get("provider_response_schema_contract_id"):
                                                                        contract_entry = contract
        assert contract_entry is not None, "Timeline did not include schema contract under pairing"
        ...
        for field in expected_lineage_fields:
            assert contract_entry.get(field), f"{field} missing from schema contract timeline entry"
            assert contract_entry.get(field) is not None
        for field in expected_false_fields:
            assert field in contract_entry, f"{field} missing from schema contract timeline entry"
            assert contract_entry[field] is False
