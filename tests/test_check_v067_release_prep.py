# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    tests/test_check_v067_release_prep.py
# PURPOSE: Verifies check v067 release prep behavior and regression
#         expectations.
...
SCRIPT = ROOT / "scripts" / "historical_release_checkers" / "check_v067_release_prep.py"
...
class TestReleasePrepMode:
    def test_release_prep_mode_fails_on_real_repo_after_bump(self) -> None:
        """Release-prep mode fails on real repo because source is now 0.6.8."""
        result = _run_script("--release-prep")
        assert result.returncode == 1, result.stdout + result.stderr
        assert "FAIL" in result.stdout

    def test_release_prep_json_fails_on_real_repo_after_bump(self) -> None:
