...
def _minimal_checker_repo(tmp_path: Path) -> Path:
    ...
    (repo / "pyproject.toml").write_text('version = "0.6.17"\n', encoding="utf-8")
    (repo / "src" / "atlas_agent" / "__init__.py").write_text('__version__ = "0.6.17"\n', encoding="utf-8")
    (repo / "docs" / "releases" / "release-metadata.json").write_text(
        json.dumps(
            {
                "source_version": "0.6.17",
                "current_public_release": "v0.6.17",
                "next_planned_release": "v0.6.18",
