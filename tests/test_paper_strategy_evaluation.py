...
class TestCheckerFailures:
    ...
    def test_checker_fails_if_release_metadata_moves_to_v0622(self, tmp_path: Path) -> None:
        tmp = _make_isolated_repo(
            tmp_path,
            metadata_patch={'"current_public_release": "v0.6.21"': '"current_public_release": "v0.6.22"'},
        )
        result = _run_isolated_checker(tmp)
        assert result.returncode == 1
