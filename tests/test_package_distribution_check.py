...
PACKAGE_VERSION = "0.5.7rc3"
PUBLIC_TAG = "v0.5.7-rc3"
...
class TestVersionReporting:
    ...
    def test_expected_tag_matches_version(self) -> None:
        text = SCRIPT.read_text(encoding="utf-8")
        assert 'EXPECTED_PUBLIC_TAG = "v0.5.7-rc3"' in text
...
class TestWheelMetadataParser:
    ...
    def test_rejects_wrong_version(self, tmp_path: Path) -> None:
        ...
        wheel_path = tmp_path / "atlas_agent-0.5.7rc2-py3-none-any.whl"
        _make_fake_wheel(wheel_path, name="atlas_agent", version="0.5.7rc2")
        ok, errors = cpd._check_wheel_metadata(wheel_path)
        assert not ok
        assert any("0.5.7rc2" in e for e in errors)
...
class TestSdistMetadataParser:
    ...
    def test_rejects_wrong_version(self, tmp_path: Path) -> None:
        ...
        sdist_path = tmp_path / "atlas-agent-0.5.7rc2.tar.gz"
        _make_fake_sdist(sdist_path, name="atlas-agent", version="0.5.7rc2")
        ok, errors = cpd._check_sdist_metadata(sdist_path)
        assert not ok
        assert any("0.5.7rc2" in e for e in errors)
...
class TestArtifactFilenameChecks:
    ...
    def test_wrong_version_fails(self) -> None:
        ...
        wheel = Path("atlas_agent-0.5.7rc2-py3-none-any.whl")
        sdist = Path("atlas-agent-0.5.7rc2.tar.gz")
        errors = cpd._check_artifact_filenames(wheel, sdist)
        assert len(errors) == 2
...
class TestRealBuild:
    def test_real_build_passes(self) -> None:
        ...
        assert "Package version: 0.5.7rc3" in result.stdout
