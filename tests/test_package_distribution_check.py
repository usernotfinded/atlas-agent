...
class TestVersionReporting:
    ...
    def test_expected_tag_matches_version(self) -> None:
        text = SCRIPT.read_text(encoding="utf-8")
        assert 'EXPECTED_PUBLIC_TAG = "v0.5.7-rc8"' in text
...
class TestWheelMetadataParser:
    def test_parses_correct_wheel(self, tmp_path: Path) -> None:
        ...
        wheel_path = tmp_path / "atlas_agent-0.5.7rc8-py3-none-any.whl"
        _make_fake_wheel(wheel_path, name="atlas_agent", version="0.5.7rc8")
...
class TestSdistMetadataParser:
    def test_parses_correct_sdist(self, tmp_path: Path) -> None:
        ...
        sdist_path = tmp_path / "atlas-agent-0.5.7rc8.tar.gz"
        _make_fake_sdist(sdist_path, name="atlas-agent", version="0.5.7rc8")
...
class TestArtifactFilenameChecks:
    def test_correct_filenames_pass(self) -> None:
        ...
        wheel = Path("atlas_agent-0.5.7rc8-py3-none-any.whl")
        sdist = Path("atlas-agent-0.5.7rc8.tar.gz")
...
class TestRealBuild:
    def test_real_build_passes(self) -> None:
        ...
        assert "Package version: 0.5.7rc8" in result.stdout
