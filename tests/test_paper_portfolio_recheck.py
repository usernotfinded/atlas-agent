...
def test_recheck_command_writes_schema(tmp_path):
    ...
    json_path = tmp_path / "paper-portfolio-recheck-ledger.json"
    md_path = tmp_path / "paper-portfolio-review-queue.md"
    ...
    assert data["overall_review_status"] in ALLOWED_RECHECK_STATUSES
    assert data["safety"]["no_notifications_sent"] is True

def test_recheck_is_deterministic(tmp_path):
    report_one = build_paper_portfolio_recheck(
        data_path=str(DATA_PATH),
        symbol="DEMO-SYMBOL",
        strategies=["buy_and_hold", "moving_average_cross"],
    )
    report_two = build_paper_portfolio_recheck(
        data_path=str(DATA_PATH),
        symbol="DEMO-SYMBOL",
        strategies=["buy_and_hold", "moving_average_cross"],
    )
    ...
    first_json, _ = write_portfolio_recheck_reports(report_one, output_dir=str(tmp_path / "one"))
    second_json, _ = write_portfolio_recheck_reports(report_two, output_dir=str(tmp_path / "two"))
...
def test_no_decision_uses_forbidden_labels():
    ...
    observed = {report["overall_review_status"]}
    observed.update(item["status"] for item in report["review_items"])
    assert not observed & FORBIDDEN_LABELS

def test_demo_script_passes():
    result = subprocess.run(
        ["bash", "scripts/demo_paper_portfolio_recheck.sh"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0, result.stdout + result.stderr
    assert "Paper portfolio recheck ledger demo PASS" in result.stdout

def test_checker_passes_on_real_repo_and_json_parses():
    result = subprocess.run(
        ["python3.11", "scripts/check_paper_portfolio_recheck.py", "--json"],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0, result.stdout + result.stderr
...
def _checker_root(tmp_path: Path) -> Path:
    ...
    os.chmod(root / "scripts/demo_paper_portfolio_recheck.sh", 0o755)
    return root
