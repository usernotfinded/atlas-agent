# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    tests/test_v062_release_prep.py
# PURPOSE: Verifies v062 release prep behavior and regression expectations.
# DEPS:    importlib, json, subprocess, sys, pathlib, types, additional local
#         modules.
# ==============================================================================

"""Tests for v0.6.2 release prep checker.
...
SCRIPT = ROOT / "scripts" / "historical_release_checkers" / "check_v062_release_prep.py"
...
def _load_script_module() -> ModuleType:
    spec = importlib.util.spec_from_file_location("check_v062_release_prep", SCRIPT)
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.modules["check_v062_release_prep"] = mod
...
class TestCheckerValid:
    ...
    def test_json_has_required_keys(self) -> None:
        result = _run_script("--json")
        data = json.loads(result.stdout)
        assert data["artifact_type"] == "v062_release_prep_report"
...
class TestCheckerNegative:
    def test_missing_version_bump_fails(self, tmp_path: Path) -> None:
        ...
        try:
            ...
            assert code == 1
            assert any("0.6.2" in e for e in result["errors"])
        finally:
    ...
    def test_v061_history_missing_fails(self, tmp_path: Path) -> None:
        mod = _load_script_module()
        original_v061_notes = mod.V061_RELEASE_NOTES
        original_v061_status = mod.V061_TRUST_STATUS
        try:
            mod.V061_RELEASE_NOTES = tmp_path / "missing-v061.md"
            mod.V061_TRUST_STATUS = tmp_path / "missing-v061-status.md"
            code, result = mod.run_check()
            assert code == 1
            assert any("v0.6.1 history missing" in e for e in result["errors"])
        finally:
            mod.V061_RELEASE_NOTES = original_v061_notes
            mod.V061_TRUST_STATUS = original_v061_status
