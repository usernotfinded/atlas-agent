...
def _load_script_module() -> ModuleType:
    ...
    sys.modules["check_v0614_post_release_hygiene"] = mod
...
def _copy_v0614_repo(tmp_path: Path) -> Path:
    ...
    # Metadata rewritten to the v0.6.14 post-cutover posture.
    metadata_path = repo / mod.METADATA
    metadata_path.parent.mkdir(parents=True, exist_ok=True)
    metadata = json.loads((ROOT / mod.METADATA).read_text(encoding="utf-8"))
    metadata["source_version"] = "0.6.14"
    metadata["current_public_release"] = "v0.6.14"
    metadata["next_planned_release"] = "v0.6.15"
    ...
    pyproject_path.write_text('version = "0.6.14"\n', encoding="utf-8")
    init_path = repo / "src/atlas_agent/__init__.py"
    init_path.parent.mkdir(parents=True, exist_ok=True)
    init_path.write_text('__version__ = "0.6.14"\n', encoding="utf-8")

    # Public-facing state docs are stubbed so the fixture is self-contained and
    # does not depend on the current repo's (post-v0.6.15) wording.
    for rel in mod.PUBLIC_STATE_DOCS:
        ...
        if rel == "README.md":
            ...
            path.write_text(
                f"# {rel}\n\n"
                "v0.6.14 is the current public release. "
                "v0.6.15 is the next planning line.\n",
                encoding="utf-8",
            )
    ...
    for rel in mod.GATE_FILES:
        path = repo / rel
        path.parent.mkdir(parents=True, exist_ok=True)
        content = "scripts/check_v0614_post_release_hygiene.py\n"
        if rel != "scripts/release_check.sh":
            content += "tests/test_v0614_post_release_hygiene.py\n"
...
def test_checker_fails_on_current_repo() -> None:
    """The historical checker correctly fails on the post-v0.6.15 repo."""
    result = _run_script()
    assert result.returncode == 1, result.stdout + result.stderr
    assert "FAIL" in result.stdout


def test_checker_passes_on_v0614_fixture(tmp_path: Path) -> None:
    repo = _copy_v0614_repo(tmp_path)
    mod = _load_script_module()
...
def test_checker_json_parses(tmp_path: Path) -> None:
    ...
    assert data["current_public_release"] == "v0.6.14"
    assert data["source_version"] == "0.6.14"
    assert data["next_planned_release"] == "v0.6.15"
...
def test_fails_on_wrong_source_version(tmp_path: Path) -> None:
    ...
    path.write_text(path.read_text(encoding="utf-8").replace('version = "0.6.14"', 'version = "0.6.15"'), encoding="utf-8")
    mod = _load_script_module()
    result = mod.check(repo)
...
def test_fails_if_previous_release_is_not_historical(tmp_path: Path) -> None:
    ...
    def mutate(data: dict) -> None:
        for release in data["releases"]:
            if release.get("tag") == "v0.6.13":
                release["status"] = "current_public"

    _mutate_json(repo, mod.METADATA, mutate)
    result = mod.check(repo)
    assert any("v0.6.13 release record must be historical" in error for error in result["errors"])


def test_fails_if_pre_cutover_doc_is_not_historical(tmp_path: Path) -> None:
    repo = _copy_v0614_repo(tmp_path)
    mod = _load_script_module()
    path = repo / "docs/releases/v0.6.14-plan.md"
    path.write_text("# v0.6.14 Release Plan\n", encoding="utf-8")
    result = mod.check(repo)
...
def test_fails_on_stale_public_release_posture(tmp_path: Path) -> None:
    ...
    mod = _load_script_module()
    path = repo / "README.md"
    path.write_text(path.read_text(encoding="utf-8") + "\nCurrent public release is v0.6.13.\n", encoding="utf-8")
    result = mod.check(repo)
