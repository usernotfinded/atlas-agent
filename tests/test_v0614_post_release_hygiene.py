"""Tests for the local v0.6.14 post-release hygiene gate."""
...
from scripts import check_v0614_post_release_hygiene as checker


ROOT = Path(__file__).resolve().parents[1]
SCRIPT = ROOT / "scripts" / "check_v0614_post_release_hygiene.py"
...
def test_fails_if_gate_integration_is_missing(tmp_path: Path) -> None:
    ...
    path.write_text(path.read_text(encoding="utf-8").replace("scripts/check_v0614_post_release_hygiene.py", "scripts/removed.py"), encoding="utf-8")
    result = checker.check(repo)
