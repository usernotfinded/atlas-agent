...
def test_success_path_summary_keys_unchanged(
    ...
    def mock_run_cmd(cmd, check=True, cwd=None, env=None):
        ...
        if "git ls-remote" in text:
            return "refs/tags/v0.6.13\n", 0, ""
        if "gh release view" in text:
            return '{"url":"https://github.com/usernotfinded/atlas-agent/releases/tag/v0.6.13"}', 0, ""
        if "update check --dry-run" in text:
            return "Current version: 0.6.13", 0, ""
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["release_assurance.py", "--version", "v0.6.13", "--output", str(tmp_path)],
    )
