...
class TestReadmePublicLaunch:
    ...
    def test_readme_contains_current_status(self, readme_text: str) -> None:
        assert "v0.6.13" in readme_text, "README must reference v0.6.13 as current source status"
...
class TestStaleRCReferencesBlocked:
    ...
    def test_current_source_version_0613_accepted(self) -> None:
        text = (
            "# README\n\n```bash\natlas --help\n```\n\n"
            "Sandbox-only, paper-first, offline-safe.\n"
            "Live trading disabled by default. Not financial advice.\n"
...
class TestReleaseDocConsistency:
    def test_public_launch_readiness_doc_has_v0613_as_current_release(self) -> None:
        text = (ROOT / "docs" / "public-launch-readiness.md").read_text(encoding="utf-8")
        assert "latest stable public GitHub release is `v0.6.13`" in text, (
            "public-launch-readiness.md must describe v0.6.13 as the current stable release"
        )
    ...
    def test_public_launch_readiness_doc_lists_v063_as_historical(self) -> None:
        text = (ROOT / "docs" / "public-launch-readiness.md").read_text(encoding="utf-8")
        assert "v0.6.3" in text and "historical" in text.lower(), (
            "public-launch-readiness.md must list v0.6.3 as historical"
        )
    ...
    def test_readme_release_assurance_example_uses_v0613(self) -> None:
        text = (ROOT / "README.md").read_text(encoding="utf-8")
        assert "--version v0.6.13" in text, (
            "README release assurance example must use v0.6.13"
        )
        assert "v0.6.1-local-check" not in text, (
            "README must not use stale v0.6.1 release assurance example"
        )

    def test_checks_reference_release_assurance_uses_v0613(self) -> None:
        text = (ROOT / "docs" / "development" / "checks-reference.md").read_text(encoding="utf-8")
        assert "--version v0.6.13" in text, (
            "checks-reference.md release assurance example must use v0.6.13"
        )
