...
class TestReleaseDocConsistency:
    def test_public_launch_readiness_doc_has_v0612_as_current_release(self) -> None:
        text = (ROOT / "docs" / "public-launch-readiness.md").read_text(encoding="utf-8")
        assert "latest stable public GitHub release is `v0.6.12`" in text, (
            "public-launch-readiness.md must describe v0.6.12 as the current stable release"
        )
    ...
    def test_readme_release_assurance_example_uses_v0612(self) -> None:
        text = (ROOT / "README.md").read_text(encoding="utf-8")
        assert "--version v0.6.12" in text, (
            "README release assurance example must use v0.6.12"
        )
        assert "v0.6.1-local-check" not in text, (
            "README must not use stale v0.6.1 release assurance example"
        )

    def test_checks_reference_release_assurance_uses_v0612(self) -> None:
        text = (ROOT / "docs" / "development" / "checks-reference.md").read_text(encoding="utf-8")
        assert "--version v0.6.12" in text, (
            "checks-reference.md release assurance example must use v0.6.12"
        )
