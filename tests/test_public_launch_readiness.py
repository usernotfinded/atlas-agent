...
class TestReadmePublicLaunch:
    ...
    def test_readme_contains_current_status(self, readme_text: str, release_identity: dict) -> None:
        assert release_identity["current_public_release"] in readme_text, (
            f"README must reference {release_identity['current_public_release']} as current source status"
        )
...
class TestStaleRCReferencesBlocked:
    ...
    def test_current_source_version_accepted(self, release_identity: dict) -> None:
        current = release_identity["current_public_release"]
        text = (
            "# README\n\n```bash\natlas --help\n```\n\n"
            "Sandbox-only, paper-first, offline-safe.\n"
            "Live trading disabled by default. Not financial advice.\n"
...
class TestReleaseDocConsistency:
    def test_public_launch_readiness_doc_has_current_release(self, release_identity: dict) -> None:
        text = (ROOT / "docs" / "public-launch-readiness.md").read_text(encoding="utf-8")
        current = release_identity["current_public_release"]
        assert f"latest stable public GitHub release is `{current}`" in text, (
            f"public-launch-readiness.md must describe {current} as the current stable release"
        )
    ...
    def test_readme_release_assurance_example_uses_current_release(self, release_identity: dict) -> None:
        text = (ROOT / "README.md").read_text(encoding="utf-8")
        current = release_identity["current_public_release"]
        assert f"--version {current}" in text, (
            f"README release assurance example must use {current}"
        )
        assert "v0.6.1-local-check" not in text, (
            "README must not use stale v0.6.1 release assurance example"
        )

    def test_checks_reference_release_assurance_uses_current_release(self, release_identity: dict) -> None:
        text = (ROOT / "docs" / "development" / "checks-reference.md").read_text(encoding="utf-8")
        current = release_identity["current_public_release"]
        assert f"--version {current}" in text, (
            f"checks-reference.md release assurance example must use {current}"
        )
