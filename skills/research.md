...
Safety rules: use Perplexity or configured research provider; avoid unsupported market claims; never make profit guarantees; never print API keys; clearly label uncertainty.

Failure modes: missing `PERPLEXITY_API_KEY`, stale data, no citations, provider timeout. If research fails, continue with local data and state the limitation.
