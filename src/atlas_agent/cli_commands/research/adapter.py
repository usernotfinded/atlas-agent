...
@research_envelope("provider-adapter-interface-contract")
def handle_provider_adapter_interface_contract(context: CLIContext, ws: Path) -> int:
    args = context.args
...
@research_envelope("provider-adapter-interface-contract-list")
def handle_provider_adapter_interface_contract_list(context: CLIContext, ws: Path) -> int:
    args = context.args
    ...
    if args.json:
        print_json({"ok": True, "status": "research_provider_adapter_interface_contract_list", "items": items})
    else:
...
@research_envelope("provider-adapter-interface-contract-show")
def handle_provider_adapter_interface_contract_show(context: CLIContext, ws: Path) -> int:
    args = context.args
...
@research_envelope("provider-adapter-interface-contract-validate")
def handle_provider_adapter_interface_contract_validate(context: CLIContext, ws: Path) -> int:
    args = context.args
    ...
    # Kept in the body rather than absorbed. This clause is the canonical pair by
    # type but not by behaviour: it returns `2 if args.strict else 1`, while the
    # envelope always returns 1. Letting the envelope take it changed the exit
    # status under `--strict`, which the envelope comparator missed because it
...
@research_envelope("provider-adapter-interface-contract-replay")
def handle_provider_adapter_interface_contract_replay(context: CLIContext, ws: Path) -> int:
    args = context.args
...
@research_envelope("provider-adapter-interface-contract-summary")
def handle_provider_adapter_interface_contract_summary(context: CLIContext, ws: Path) -> int:
    args = context.args
...
@research_envelope("provider-adapter-interface-contract-doctor")
def handle_provider_adapter_interface_contract_doctor(context: CLIContext, ws: Path) -> int:
    args = context.args
...
@research_envelope("provider-adapter-disabled-smoke")
def handle_provider_adapter_disabled_smoke(context: CLIContext, ws: Path) -> int:
    args = context.args
