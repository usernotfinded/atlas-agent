...
@research_envelope("market")
def handle_market(context: CLIContext, ws: Path) -> int:
    args = context.args
    if args.json:
...
def handle_timeline(context: CLIContext, ws: Path) -> int:
    ...
    try:
        ...
        if limit < 1:
            if args.json:
                import json
                print_json({"ok": False, "status": "invalid_limit"})
            else:
...
@research_envelope("simulate-provider")
def handle_simulate_provider(context: CLIContext, ws: Path) -> int:
    args = context.args
    import json
    from atlas_agent.events import EventLogger
    from atlas_agent.research.errors import safe_research_session_error
...
@research_envelope("import-provider-response")
def handle_import_provider_response(context: CLIContext, ws: Path) -> int:
    args = context.args
    import json
