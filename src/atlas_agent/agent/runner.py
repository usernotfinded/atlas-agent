...
def run_agent(mode: str, config: AtlasConfig, models: str | None = None) -> RoutineResult:
    ...
    if mode == "auto":
        ...
        if state == "open":
            ...
            return run_open_market_cycle(config, mode=run_mode, models=models)
        elif state in ("closed", "premarket", "afterhours", "weekend", "holiday"):
            return run_closed_market_cycle(config, mode="paper", models=models)
        else:
        ...
        if state == "open":
            return run_open_market_cycle(config, mode="paper", models=models)
        else:
            return run_closed_market_cycle(config, mode="paper", models=models)
        ...
        if state == "open":
            if not config.enable_live_trading:
                ...
                return run_open_market_cycle(config, mode="live", models=models)
            return run_open_market_cycle(config, mode="live", models=models)
        else:
