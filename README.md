...
> **Current Status (v0.6.18)** — package/source version is `0.6.18`. `v0.6.18` is the current public GitHub release. `v0.6.17` is the historical previous public release. `v0.6.19` is the next planning line. Historical stable baseline is `v0.5.8`. PyPI was not published. See [CHANGELOG.md](CHANGELOG.md) for full release history.
...
`v0.6.18` is the current public GitHub release. `v0.6.17` is the historical previous public release. `v0.6.19` is the next planning line. The package/source version on main is `0.6.18`. Provider execution remains locked. Trust remains blocked. Live submit remains disabled by default. No profitability or trading correctness claims.
...
```bash
python scripts/release_assurance.py --version v0.6.18 --output artifacts/release_assurance/v0.6.18-local-check
```
