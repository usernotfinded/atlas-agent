...
- PR #9, `Docs: record Batch 7 post-incident audit`, was reported as merged by GitHub metadata, but current `refs/heads/main` still pointed to `3491a9b` during follow-up verification. This note is therefore reintroduced from the current effective `main` state.
