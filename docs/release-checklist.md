...
```bash
./scripts/smoke_release_tag.sh v0.5.7.dev37
```

Optional full mode (also runs `release_check.sh` inside the clean clone):

```bash
./scripts/smoke_release_tag.sh v0.5.7.dev37 --full
```
...
git tag -a v0.5.7.dev37 -m "Atlas Agent v0.5.7.dev37"
git push origin v0.5.7.dev37
```
