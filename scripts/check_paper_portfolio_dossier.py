...
# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    scripts/check_paper_portfolio_dossier.py
# PURPOSE: Validates paper portfolio dossier requirements for local and CI
#         workflows.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if issues:
        if args.json:
            ...
            print("Paper portfolio dossier check FAILED")
            for issue in issues:
                print(f"  - {issue}")
        return 1
    ...
    if args.json:
        ...
        print("Paper portfolio dossier check PASSED")
    return 0
...
def _check_demo_script(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "scripts/demo_paper_portfolio_dossier.sh"
    if not path.exists():
        return issues
    ...
    if "portfolio-dossier" not in content:
        issues.append("Demo script must use the paper portfolio dossier command")
...
def _check_docs(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "docs/paper-portfolio-dossier.md"
    if not path.exists():
        return issues
    ...
    required_phrases = [
        ...
        "paper-portfolio-dossier.json",
        "paper-portfolio-dossier.md",
        "paper-portfolio-evidence-manifest.json",
        "no provider calls",
        "no broker calls",
        "no credentials",
        "no live trading",
    ...
    for decision in ALLOWED_DECISIONS:
        if decision not in text:
            issues.append(f"Docs missing allowed dossier decision: {decision}")
...
def _check_candidate_docs(root: Path) -> list[str]:
    ...
    for path in plan_paths:
        ...
        content = path.read_text(encoding="utf-8")
        lower = content.lower()
        if "cand-005" not in lower:
            issues.append(f"CAND-005 missing from {path.relative_to(root)}")
    ...
    if json_path.exists():
        ...
        cand = next((item for item in candidates if item.get("id") == "CAND-005"), None)
        if not cand:
            issues.append("CAND-005 missing from v0.6.14 candidates JSON")
        elif cand.get("status") not in {"implemented", "current"}:
            issues.append("CAND-005 must be listed as implemented/current in candidates JSON")
    return issues
