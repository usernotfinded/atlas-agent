...
def _check_release_metadata(root: Path) -> list[str]:
    ...
    if 'version = "0.6.13"' not in pyproject:
        issues.append("pyproject.toml source/package version must remain 0.6.13")
    if '__version__ = "0.6.13"' not in init_file:
        issues.append("src/atlas_agent/__init__.py version must remain 0.6.13")
