...
PYPI_PUBLISHED = _meta.pypi_published

ISOLATION_DOC = REPO_ROOT / "docs" / "paper-provider-isolation.md"
AUTONOMOUS_DOC = REPO_ROOT / "docs" / "autonomous-paper-workflow.md"
GOVERNANCE_DOC = REPO_ROOT / "docs" / "bounded-live-autonomy-governance.md"
LIVE_SUBMIT_DOC = REPO_ROOT / "docs" / "live-submit-safety-contract.md"
PAPER_GUIDE_DOC = REPO_ROOT / "docs" / "paper-trading-guide.md"
...
FACTORY_SOURCE = REPO_ROOT / "src" / "atlas_agent" / "providers" / "factory.py"
RUNNER_SOURCE = REPO_ROOT / "src" / "atlas_agent" / "agent" / "runner.py"
CLI_SOURCE = REPO_ROOT / "src" / "atlas_agent" / "cli.py"
TEST_FILE = REPO_ROOT / "tests" / "test_paper_provider_isolation.py"
...
REQUIRED_LINKS_FROM_ISOLATION_DOC = [
    "autonomous-paper-workflow.md",
    "bounded-live-autonomy-governance.md",
    "live-submit-safety-contract.md",
    "paper-trading-guide.md",
...
def _check_demo_script() -> list[str]:
    ...
    for cmd in REQUIRED_DEMO_COMMANDS:
        if cmd not in text:
...
def _check_forbidden_doc_claims() -> list[str]:
    errors: list[str] = []
    for path in [ISOLATION_DOC, AUTONOMOUS_DOC, GOVERNANCE_DOC]:
        if not path.exists():
            continue
...
def _check_cross_references() -> list[str]:
    errors: list[str] = []
    if AUTONOMOUS_DOC.exists():
        text = _read(AUTONOMOUS_DOC)
        if "paper-provider-isolation.md" not in text:
            errors.append("[docs/autonomous-paper-workflow.md] Missing link to paper-provider-isolation.md")

    if ISOLATION_DOC.exists():
        text = _read(ISOLATION_DOC)
        for link in REQUIRED_LINKS_FROM_ISOLATION_DOC:
            if link not in text:
                errors.append(f"[docs/paper-provider-isolation.md] Missing link to {link}")
    ...
    for path, label in [
        ...
        text = _read(path)
        if "paper-provider-isolation.md" not in text:
            errors.append(f"[{label}] Missing link to paper-provider-isolation.md")

    return errors


def _check_implementation_wiring() -> list[str]:
    errors: list[str] = []

    if not FACTORY_SOURCE.exists():
        errors.append("Provider factory source missing")
    else:
        text = _read(FACTORY_SOURCE)
        if "NullProvider" not in text:
            errors.append("[providers/factory.py] NullProvider not referenced")
        if 'provider_id == "null"' not in text:
            errors.append('[providers/factory.py] Missing "null" provider branch')

    if not RUNNER_SOURCE.exists():
        errors.append("Agent runner source missing")
    else:
        text = _read(RUNNER_SOURCE)
        if "get_provider_from_runtime_config(config, mode=effective_mode)" not in text:
            errors.append("[agent/runner.py] Provider resolution does not pass effective mode")

    if not CLI_SOURCE.exists():
        errors.append("CLI source missing")
    else:
        text = _read(CLI_SOURCE)
        if '"--offline"' not in text:
            errors.append('[cli.py] Missing --offline flag')

    return errors


def _check_tests_mention_flag() -> list[str]:
    errors: list[str] = []
    if TEST_FILE.exists():
        text = _read(TEST_FILE)
        if '"--offline"' not in text:
            errors.append("[tests/test_paper_provider_isolation.py] Missing --offline flag coverage")
    return errors


def _check_release_metadata() -> list[str]:
    errors: list[str] = []
    if PACKAGE_VERSION != "0.6.20":
        errors.append(
            f"Source version {PACKAGE_VERSION} != 0.6.20"
        )
    if CURRENT_PUBLIC_TAG != "v0.6.20":
        errors.append(f"Current public release {CURRENT_PUBLIC_TAG} != v0.6.20")
    if NEXT_PLANNED_TAG not in ("v0.6.21", "0.6.21"):
        errors.append(f"Next planned release {NEXT_PLANNED_TAG} != v0.6.21")
    if PYPI_PUBLISHED is True:
        errors.append("PyPI is marked as published; must remain false")

    result = subprocess.run(
        ["git", "tag", "--list", "v0.6.21"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
    )
    if result.stdout.strip():
        errors.append("Local git tag v0.6.21 already exists")
...
def _run_checks() -> dict:
    ...
    errors.extend(_check_implementation_wiring())
    errors.extend(_check_tests_mention_flag())
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Paper-mode provider isolation and no-network gate check."
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    if result["errors"]:
        print("Paper provider isolation check FAILED")
        for e in result["errors"]:
            print(f"  - {_redact(e)}")
        return 1

    print("Paper provider isolation check PASSED")
