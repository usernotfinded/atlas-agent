...
def _check_release_metadata(root: Path) -> list[str]:
    ...
    if 'version = "0.6.14"' not in pyproject:
        issues.append("pyproject.toml source/package version must be 0.6.14")
    if '__version__ = "0.6.14"' not in init_file:
        issues.append("src/atlas_agent/__init__.py version must be 0.6.14")
