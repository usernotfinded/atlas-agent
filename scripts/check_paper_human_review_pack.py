...
# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    scripts/check_paper_human_review_pack.py
# PURPOSE: Validates paper human review pack requirements for local and CI
#         workflows.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if issues:
        if args.json:
            ...
            print("Paper human review pack check FAILED")
            for issue in issues:
                print(f"  - {issue}")
        return 1
    ...
    if args.json:
        ...
        print("Paper human review pack check PASSED")
    return 0
...
def _check_demo_script(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "scripts/demo_paper_human_review_pack.sh"
    if not path.exists():
        return issues
    ...
    if "portfolio-review-pack" not in content:
        issues.append("Demo script must use the paper human review pack command")
...
def _check_docs(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "docs/paper-human-review-pack.md"
    if not path.exists():
        return issues
    ...
    for status in ALLOWED_REVIEW_PACK_STATUSES:
        if status not in text:
            issues.append(f"Docs missing allowed review pack status: {status}")
    for status in ALLOWED_REVIEW_ITEM_STATUSES:
        if status not in text:
            issues.append(f"Docs missing allowed review item status: {status}")
...
def _check_release_metadata(root: Path) -> list[str]:
    ...
    if 'version = "0.6.16"' not in pyproject and 'version = "0.6.17"' not in pyproject and 'version = "0.6.18"' not in pyproject and 'version = "0.6.19"' not in pyproject and 'version = "0.6.20"' not in pyproject and 'version = "0.6.21"' not in pyproject and 'version = "0.6.24"' not in pyproject and 'version = "0.6.25"' not in pyproject and 'version = "0.6.26"' not in pyproject and 'version = "0.6.27"' not in pyproject:
        issues.append("pyproject.toml source/package version must be 0.6.16, 0.6.17, 0.6.18, 0.6.19, 0.6.20, 0.6.21, 0.6.24, 0.6.25, 0.6.26, or 0.6.27")
    if '__version__ = "0.6.16"' not in init_file and '__version__ = "0.6.17"' not in init_file and '__version__ = "0.6.18"' not in init_file and '__version__ = "0.6.19"' not in init_file and '__version__ = "0.6.20"' not in init_file and '__version__ = "0.6.21"' not in init_file and '__version__ = "0.6.24"' not in init_file and '__version__ = "0.6.25"' not in init_file and '__version__ = "0.6.26"' not in init_file and '__version__ = "0.6.27"' not in init_file:
        issues.append("src/atlas_agent/__init__.py version must be 0.6.16, 0.6.17, 0.6.18, 0.6.19, 0.6.20, 0.6.21, 0.6.24, 0.6.25, 0.6.26, or 0.6.27")
...
def _check_candidate_docs(root: Path) -> list[str]:
    ...
    if json_path.exists():
        ...
        cand = next((item for item in candidates if item.get("id") == "CAND-001"), None)
        if not cand:
            issues.append("CAND-001 missing from v0.6.15 candidates JSON")
        elif cand.get("status") not in {"proposed", "implemented"}:
            issues.append("CAND-001 must be listed as proposed/implemented in candidates JSON")
    return issues
...
def _check_cli_command(root: Path) -> list[str]:
    ...
    if "portfolio-review-pack" not in content:
        issues.append("CLI must register portfolio-review-pack command")
    if "build_paper_portfolio_review_pack" not in content:
        issues.append("CLI must import build_paper_portfolio_review_pack")
    if "write_portfolio_review_pack_reports" not in content:
        issues.append("CLI must import write_portfolio_review_pack_reports")
    return issues
