...
# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    scripts/check_paper_portfolio_stress.py
# PURPOSE: Validates paper portfolio stress requirements for local and CI
#         workflows.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if issues:
        if args.json:
            ...
            print("Paper portfolio stress check FAILED")
            for issue in issues:
                print(f"  - {issue}")
        return 1
    ...
    if args.json:
        ...
        print("Paper portfolio stress check PASSED")
    return 0
...
def _check_demo_script(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "scripts/demo_paper_portfolio_stress.sh"
    if not path.exists():
        return issues
    ...
    if "portfolio-stress" not in content:
        issues.append("Demo script must use the paper portfolio stress command")
...
def _check_docs(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "docs/paper-portfolio-stress.md"
    if not path.exists():
        return issues
    ...
    required_phrases = [
        ...
        "sideways_chop",
        "slow_drawdown",
        "paper-portfolio-stress.json",
        "paper-portfolio-stress.md",
        "no provider calls",
        "no broker calls",
        "no credentials",
        "no live trading",
    ...
    for decision in ALLOWED_DECISIONS:
        if decision not in text:
            issues.append(f"Docs missing allowed stress decision: {decision}")
...
def _check_candidate_docs(root: Path) -> list[str]:
    ...
    if json_path.exists():
        ...
        cand = next((item for item in candidates if item.get("id") == "CAND-002"), None)
        if not cand:
            issues.append("CAND-002 missing from v0.6.14 candidates JSON")
        elif cand.get("status") not in {"implemented", "current"}:
            issues.append("CAND-002 must be listed as implemented/current in candidates JSON")
    return issues
