...
# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    scripts/check_paper_human_review_policy.py
# PURPOSE: Validates paper human review policy requirements for local and CI
#         workflows.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if issues:
        if args.json:
            ...
            print("Paper human review policy check FAILED")
            for issue in issues:
                print(f"  - {issue}")
        return 1
    ...
    if args.json:
        ...
        print("Paper human review policy check PASSED")
    return 0
...
def _check_demo_script(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "scripts/demo_paper_human_review_policy.sh"
    if not path.exists():
        return issues
    ...
    if "portfolio-review-policy" not in content:
        issues.append("Demo script must use the paper human review policy command")
...
def _check_docs(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "docs/paper-human-review-policy.md"
    if not path.exists():
        return issues
    ...
    for status in ALLOWED_REVIEW_POLICY_STATUSES:
        if status not in text:
            issues.append(f"Docs missing allowed review policy status: {status}")
    for state in ALLOWED_POLICY_RESULT_STATES:
        if state not in text:
            issues.append(f"Docs missing allowed policy result state: {state}")
...
def _check_candidate_docs(root: Path) -> list[str]:
    ...
    for path in plan_paths:
        ...
        if "cand-003" not in lower:
            issues.append(f"CAND-003 missing from {path.relative_to(root)}")
...
def _check_cli_command(root: Path) -> list[str]:
    ...
    if "portfolio-review-policy" not in content:
        issues.append("CLI must register portfolio-review-policy command")
    if "build_paper_portfolio_review_policy" not in content:
        issues.append("CLI must import build_paper_portfolio_review_policy")
    if "write_portfolio_review_policy_reports" not in content:
        issues.append("CLI must import write_portfolio_review_policy_reports")
    return issues
