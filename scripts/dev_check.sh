...
"$PYTHON_BIN" scripts/check_paper_human_review_ledger.py
"$PYTHON_BIN" scripts/check_paper_human_review_policy.py
"$PYTHON_BIN" scripts/check_paper_human_review_replay.py
"$PYTHON_BIN" scripts/check_v0615_paper_human_review_evidence.py
"$PYTHON_BIN" scripts/check_v0615_final_readiness_audit.py
TOTAL_ELAPSED=$((TOTAL_ELAPSED + SECONDS))
echo "  → elapsed: ${SECONDS}s"

echo ""
echo "4m. autonomous paper loop contract check"
echo "========================================"
SECONDS=0
...
echo ""
echo "4n. autonomous paper loop tests"
echo "========================================"
SECONDS=0
"$PYTHON_BIN" -m pytest tests/test_autonomous_paper_loop.py tests/test_autonomous_paper_loop_contract.py tests/test_shadow_live_contract.py -q "${PYTEST_EXTRA_ARGS[@]}"
TOTAL_ELAPSED=$((TOTAL_ELAPSED + SECONDS))
echo "  → elapsed: ${SECONDS}s"

echo ""
echo "4o. autonomous paper scorecard contract check"
echo "========================================"
SECONDS=0
...
echo ""
echo "4p. autonomous paper scorecard tests"
echo "========================================"
SECONDS=0
...
echo ""
echo "[dev-check] CAND-005 shadow-live read-only contract"
SECONDS=0
...
echo ""
echo "[dev-check] CAND-006 gated submit conformance rehearsal"
SECONDS=0
...
echo ""
echo "[dev-check] CAND-007 runtime readiness envelope evaluation"
SECONDS=0
...
echo ""
echo "[dev-check] CAND-008 operator approval gate evaluation"
SECONDS=0
...
echo ""
echo "13.9. v0.6.15 post-release hygiene check"
SECONDS=0
"$PYTHON_BIN" scripts/check_v0615_post_release_hygiene.py
TOTAL_ELAPSED=$((TOTAL_ELAPSED + SECONDS))
echo "  → elapsed: ${SECONDS}s"

echo ""
echo "13.10. v0.6.15 post-release hygiene tests"
SECONDS=0
"$PYTHON_BIN" -m pytest tests/test_v0615_post_release_hygiene.py -q "${PYTEST_EXTRA_ARGS[@]}"
TOTAL_ELAPSED=$((TOTAL_ELAPSED + SECONDS))
echo "  → elapsed: ${SECONDS}s"
