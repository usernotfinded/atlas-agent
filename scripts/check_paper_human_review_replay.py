...
# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    scripts/check_paper_human_review_replay.py
# PURPOSE: Validates paper human review replay requirements for local and CI
#         workflows.
...
def main(argv: list[str] | None = None) -> int:
    ...
    if issues:
        if args.json:
            ...
            print("Paper human review replay check FAILED")
            for issue in issues:
                print(f"  - {issue}")
        return 1
    ...
    if args.json:
        ...
        print("Paper human review replay check PASSED")
    return 0
...
def _check_demo_script(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "scripts/demo_paper_human_review_replay.sh"
    if not path.exists():
        return issues
    ...
    if "portfolio-review-replay" not in content:
        issues.append("Demo script must use the paper human review replay command")
...
def _check_docs(root: Path) -> list[str]:
    issues: list[str] = []
    path = root / "docs/paper-human-review-replay.md"
    if not path.exists():
        return issues
    ...
    for status in ALLOWED_REVIEW_REPLAY_STATUSES:
        if status not in text:
            issues.append(f"Docs missing allowed review replay status: {status}")
...
def _check_cli_command(root: Path) -> list[str]:
    ...
    if "portfolio-review-replay" not in content:
        issues.append("CLI must register portfolio-review-replay command")
    if "build_paper_portfolio_review_replay" not in content:
        issues.append("CLI must import build_paper_portfolio_review_replay")
    if "write_portfolio_review_replay_reports" not in content:
        issues.append("CLI must import write_portfolio_review_replay_reports")
    return issues
