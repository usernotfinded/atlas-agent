...
# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    scripts/check_v0615_paper_human_review_evidence.py
# PURPOSE: Check the v0.6.15 paper human review evidence bundle and candidate
#         closure gate.
...
POST_V0621_SOURCE_VERSION = "0.6.21"
POST_V0624_SOURCE_VERSION = "0.6.24"
POST_V0625_SOURCE_VERSION = "0.6.25"
POST_V0626_SOURCE_VERSION = "0.6.26"
POST_V0627_SOURCE_VERSION = "0.6.27"
...
def _check_repository_version(root: Path, errors: list[str]) -> None:
    ...
    source_ok = (
        ...
        f'version = "{POST_V0617_SOURCE_VERSION}"' in pyproject
        and f'__version__ = "{POST_V0617_SOURCE_VERSION}"' in init_py
    ) or (
        f'version = "0.6.18"' in pyproject
        and f'__version__ = "0.6.18"' in init_py
    ) or (
        f'version = "0.6.19"' in pyproject
        and f'__version__ = "0.6.19"' in init_py
    ) or (
        f'version = "0.6.20"' in pyproject
        and f'__version__ = "0.6.20"' in init_py
    ) or (
        f'version = "{POST_V0621_SOURCE_VERSION}"' in pyproject
        and f'__version__ = "{POST_V0621_SOURCE_VERSION}"' in init_py
    ) or (
        f'version = "{POST_V0624_SOURCE_VERSION}"' in pyproject
        and f'__version__ = "{POST_V0624_SOURCE_VERSION}"' in init_py
    ) or (
        f'version = "{POST_V0625_SOURCE_VERSION}"' in pyproject
        and f'__version__ = "{POST_V0625_SOURCE_VERSION}"' in init_py
    ) or (
        f'version = "{POST_V0626_SOURCE_VERSION}"' in pyproject
        and f'__version__ = "{POST_V0626_SOURCE_VERSION}"' in init_py
    ) or (
        f'version = "{POST_V0627_SOURCE_VERSION}"' in pyproject
        and f'__version__ = "{POST_V0627_SOURCE_VERSION}"' in init_py
    )
    if not source_ok:
        errors.append("Source/package version must match the audited v0.6.15 state (0.6.14, 0.6.15, 0.6.16, 0.6.17, 0.6.18, 0.6.19, 0.6.20, 0.6.21, 0.6.24, 0.6.25, 0.6.26, or 0.6.27)")
    ...
    metadata_ok = (
        ...
        '"current_public_release": "v0.6.17"' in release_metadata
        and '"next_planned_release": "v0.6.18"' in release_metadata
    ) or (
        '"current_public_release": "v0.6.18"' in release_metadata
        and '"next_planned_release": "v0.6.19"' in release_metadata
    ) or (
        '"current_public_release": "v0.6.19"' in release_metadata
        and '"next_planned_release": "v0.6.20"' in release_metadata
    ) or (
        '"current_public_release": "v0.6.20"' in release_metadata
        and '"next_planned_release": "v0.6.21"' in release_metadata
    ) or (
        '"current_public_release": "v0.6.21"' in release_metadata
        and '"next_planned_release": "v0.6.22"' in release_metadata
    ) or (
        '"current_public_release": "v0.6.24"' in release_metadata
        and '"next_planned_release": "v0.6.25"' in release_metadata
    ) or (
        '"current_public_release": "v0.6.25"' in release_metadata
        and '"next_planned_release": "v0.6.26"' in release_metadata
    ) or (
        '"current_public_release": "v0.6.26"' in release_metadata
        and '"next_planned_release": "v0.6.27"' in release_metadata
    ) or (
        '"current_public_release": "v0.6.27"' in release_metadata
        and '"next_planned_release": "v0.6.28"' in release_metadata
    )
    if not metadata_ok:
