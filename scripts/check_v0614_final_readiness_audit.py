...
# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    scripts/check_v0614_final_readiness_audit.py
# PURPOSE: Check the v0.6.14 final paper portfolio readiness audit.
# DEPS:    argparse, json, re, sys, pathlib, typing.
# ==============================================================================

"""Check the v0.6.14 final paper portfolio readiness audit.
...
ARTIFACT_TYPE = "v0614_final_readiness_audit"
...
POST_V0615_CURRENT = "v0.6.15"
POST_V0615_NEXT = "v0.6.16"
POST_V0615_SOURCE = "0.6.15"
...
POST_V0618_CURRENT = "v0.6.18"
POST_V0618_NEXT = "v0.6.19"
POST_V0618_SOURCE = "0.6.18"
POST_V0619_CURRENT = "v0.6.19"
POST_V0619_NEXT = "v0.6.20"
POST_V0619_SOURCE = "0.6.19"
POST_V0620_CURRENT = "v0.6.20"
POST_V0620_NEXT = "v0.6.21"
POST_V0620_SOURCE = "0.6.20"
POST_V0621_CURRENT = "v0.6.21"
POST_V0621_NEXT = "v0.6.22"
POST_V0621_SOURCE = "0.6.21"
POST_V0624_CURRENT = "v0.6.24"
POST_V0624_NEXT = "v0.6.25"
POST_V0624_SOURCE = "0.6.24"
POST_V0625_CURRENT = "v0.6.25"
POST_V0625_NEXT = "v0.6.26"
POST_V0625_SOURCE = "0.6.25"
POST_V0626_CURRENT = "v0.6.26"
POST_V0626_NEXT = "v0.6.27"
POST_V0626_SOURCE = "0.6.26"
POST_V0627_CURRENT = "v0.6.27"
POST_V0627_NEXT = "v0.6.28"
POST_V0627_SOURCE = "0.6.27"
...
REQUIRED_CHECK_FRAGMENTS = [
    ...
    "scripts/check_paper_portfolio_replay.py",
    "scripts/check_v0614_paper_portfolio_evidence.py",
    "scripts/check_v0614_final_readiness_audit.py",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check v0.6.14 final readiness audit.")
...
def _check_schema(data: dict[str, Any], errors: list[str]) -> None:
    ...
    for key, value in expected.items():
        if data.get(key) != value:
            errors.append(f"Audit JSON {key} must be {value!r}")
...
def _check_candidates(data: dict[str, Any], errors: list[str]) -> None:
    ...
    final = by_id["CAND-008"]
    final_title = str(final.get("title", "")).lower()
    if "final paper portfolio release-readiness audit" not in final_title:
        errors.append("Audit JSON CAND-008 must be represented as the final readiness audit")
    if final.get("status") != "complete":
        errors.append("Audit JSON CAND-008 status must be 'complete'")
    if final.get("mode") != "paper":
        errors.append("Audit JSON CAND-008 mode must be 'paper'")
...
def _check_markdown(path: Path, errors: list[str]) -> None:
    ...
    _scan_phrases(AUDIT_MD, text, FORBIDDEN_RELEASE_CLAIMS, "v0.6.14 release claim", errors)
...
def _check_repository_posture(root: Path, errors: list[str]) -> None:
    ...
    allowed = {
        ...
        (POST_V0615_SOURCE, POST_V0615_CURRENT, POST_V0615_NEXT),
        (POST_V0616_SOURCE, POST_V0616_CURRENT, POST_V0616_NEXT),
        (POST_V0617_SOURCE, POST_V0617_CURRENT, POST_V0617_NEXT),
        (POST_V0618_SOURCE, POST_V0618_CURRENT, POST_V0618_NEXT),
        (POST_V0619_SOURCE, POST_V0619_CURRENT, POST_V0619_NEXT),
        (POST_V0620_SOURCE, POST_V0620_CURRENT, POST_V0620_NEXT),
        (POST_V0621_SOURCE, POST_V0621_CURRENT, POST_V0621_NEXT),
        (POST_V0624_SOURCE, POST_V0624_CURRENT, POST_V0624_NEXT),
        (POST_V0625_SOURCE, POST_V0625_CURRENT, POST_V0625_NEXT),
        (POST_V0626_SOURCE, POST_V0626_CURRENT, POST_V0626_NEXT),
        (POST_V0627_SOURCE, POST_V0627_CURRENT, POST_V0627_NEXT),
    }
...
def _check_gate_integration(root: Path, errors: list[str]) -> None:
    for rel in GATE_FILES:
        ...
        for fragment in GATE_REQUIRED_FRAGMENTS:
            if rel == "scripts/release_check.sh" and fragment.startswith("tests/"):
                # release_check.sh --quick delegates to dev_check.sh, but full mode
                # should still keep the checker visible in its direct test subset.
                pass
...
def _payload(*, valid: bool, errors: list[str], warnings: list[str]) -> dict[str, Any]:
    return {
        "artifact_type": "v0614_final_readiness_audit_check",
        "schema_version": 1,
        "valid": valid,
        "release": NEXT_PLANNED,
        "current_public_release": CURRENT_PUBLIC,
...
def _emit(payload: dict[str, Any], *, json_output: bool) -> None:
    if json_output:
        ...
        return
    if payload["valid"]:
        print("v0.6.14 final readiness audit check PASSED")
        return
    print("v0.6.14 final readiness audit check FAILED")
