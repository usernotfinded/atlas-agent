#!/usr/bin/env python3
"""Check the CAND-028 paper strategy walk-forward surface.
...
CURRENT_PUBLIC = "v0.6.19"
NEXT_PLANNED = "v0.6.20"
...
WINDOW_FIXTURES = [
    "data/sample/windows/ohlcv_uptrend.csv",
    "data/sample/windows/ohlcv_downtrend.csv",
    "data/sample/windows/ohlcv_flat.csv",
    "data/sample/windows/ohlcv_volatile.csv",
]
ALLOWED_DECISIONS = {
...
def _check_required_files(root: Path, errors: list[str]) -> None:
    ...
    demo = root / "scripts" / "demo_paper_strategy_walk_forward.sh"
    checker = root / "scripts" / "check_paper_strategy_walk_forward.py"
    if not demo.exists():
        errors.append("Missing demo script: scripts/demo_paper_strategy_walk_forward.sh")
    if not checker.exists():
        errors.append("Missing checker: scripts/check_paper_strategy_walk_forward.py")


def _check_fixtures(root: Path, errors: list[str]) -> None:
    expected_header = ["timestamp", "open", "high", "low", "close", "volume"]
    path = root / "data" / "sample" / "ohlcv_extended.csv"
    rel = path.relative_to(root)
    if not path.exists():
        errors.append(f"Missing required window fixture: {rel}")
        return
    import csv
    try:
...
def _check_demo_script(root: Path, errors: list[str]) -> None:
    path = root / "scripts" / "demo_paper_strategy_walk_forward.sh"
    if not path.exists():
        return
    ...
    required = [
        ...
        "strategy-walk-forward.json",
        "strategy-walk-forward.md",
        "data/sample/ohlcv_extended.csv",
        "demo-symbol",
    ]
    ...
    if "strategy-sensitivity" in lower:
        errors.append("Demo script must run the walk-forward command, not sensitivity output checks")


def _check_docs(root: Path, errors: list[str]) -> None:
    doc = root / "docs" / "paper-strategy-walk-forward.md"
    if not doc.exists():
        return
    text = _read(doc)
    lower = text.lower()
    for topic in REQUIRED_DOC_TOPICS:
        if topic not in lower:
            errors.append(f"Walk-Forward doc missing required topic: {topic}")
    for decision in ALLOWED_DECISIONS:
        if decision not in text:
            errors.append(f"Walk-Forward doc missing allowed decision: {decision}")
    for decision in FORBIDDEN_DECISIONS:
        if decision in text:
            errors.append(f"Walk-Forward doc contains forbidden decision: {decision}")
    for claim in FORBIDDEN_DOC_CLAIMS:
        for match in re.finditer(re.escape(claim), lower):
            if not _is_negated(lower, match.start()):
                errors.append(f"Forbidden walk-forward doc claim: {claim}")
                break
    required_links = [
    ...
    for link in required_links:
        if link not in lower:
            errors.append(f"Walk-Forward doc missing relationship link: {link}")
...
def _check_release_metadata(root: Path, errors: list[str]) -> None:
    ...
    if SOURCE_VERSION not in pyproject:
        errors.append("Source/package version must be 0.6.19 in pyproject.toml")
    if SOURCE_VERSION not in init_py:
        errors.append("Source/package version must be 0.6.19 in src/atlas_agent/__init__.py")
...
def _check_candidate_docs(root: Path, errors: list[str], warnings: list[str]) -> None:
    ...
    if "cand-027" not in lower:
        errors.append("CAND-028 must be listed in v0.6.13 candidate planning docs")
    if "paper strategy walk-forward" not in lower:
        errors.append("Candidate docs must mention paper strategy walk-forward")
    for phrase in [
        "v0.6.13 is released",
    ...
    if not any(item.get("id") == "CAND-028" and item.get("implemented") is True for item in candidates):
        errors.append("Candidate inventory must list implemented CAND-028")
    if not candidates:
...
def _payload(*, valid: bool, errors: list[str], warnings: list[str] | None = None) -> dict[str, Any]:
    return {
        "artifact_type": "paper_strategy_walk-forward_check",
        "schema_version": 1,
        "valid": valid,
...
def _emit(payload: dict[str, Any], *, json_output: bool) -> None:
    if json_output:
        ...
        return
    if payload["valid"]:
        print("Paper strategy walk-forward check PASSED")
        return
    print("Paper strategy walk-forward check FAILED")
