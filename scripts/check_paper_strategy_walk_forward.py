...
def _check_release_metadata(root: Path, errors: list[str]) -> None:
    ...
    if SOURCE_VERSION not in pyproject:
        errors.append("Source/package version must be 0.6.16 in pyproject.toml")
    if SOURCE_VERSION not in init_py:
        errors.append("Source/package version must be 0.6.16 in src/atlas_agent/__init__.py")
