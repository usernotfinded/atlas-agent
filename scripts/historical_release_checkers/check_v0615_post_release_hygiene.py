...
PREVIOUS_PUBLIC = "v0.6.14"
NEXT_PLANNED = "v0.6.16"

METADATA = "docs/releases/release-metadata.json"
EVIDENCE_JSON = "docs/releases/v0.6.15-post-release-evidence.json"
EVIDENCE_MD = "docs/releases/v0.6.15-post-release-evidence.md"
RELEASE_NOTES = "docs/releases/v0.6.15.md"
TRUST_STATUS = "docs/trust/v0.6.15-status.md"
...
def _check_metadata(root: Path, errors: list[str]) -> None:
    ...
    for field, value in current_expected.items():
        if current.get(field) != value:
            errors.append(f"v0.6.15 release record {field} must be {value!r}")
    if previous.get("status") != "historical":
        errors.append("v0.6.14 release record must be historical")
    if any(item.get("status") == "current_public" for tag, item in records.items() if tag != CURRENT_PUBLIC):
        errors.append("only v0.6.15 may be current_public")


def _check_source_version(root: Path, errors: list[str]) -> None:
    pyproject = _read(root, "pyproject.toml")
    init_py = _read(root, "src/atlas_agent/__init__.py")
    if not re.search(r'^version\s*=\s*"0\.6\.15"$', pyproject, re.M):
        errors.append("pyproject.toml project.version must be 0.6.15")
    if not re.search(r'^__version__\s*=\s*"0\.6\.15"$', init_py, re.M):
        errors.append("src/atlas_agent/__init__.py __version__ must be 0.6.15")
...
def _check_public_docs(root: Path, errors: list[str]) -> None:
    for rel in PUBLIC_STATE_DOCS:
        ...
        lower = text.lower()
        stale_patterns = [
            r"current public (?:github )?release (?:is )?`?v0\.6\.14",
            r"latest stable public github release is v0\.6\.14",
            r"next planning line (?:is )?`?v0\.6\.15",
            r"next planned release:?\s*`?v0\.6\.15",
        ]
...
def _check_gate_integration(root: Path, errors: list[str]) -> None:
    for rel in GATE_FILES:
        text = _read(root, rel)
        if "scripts/check_v0615_post_release_hygiene.py" not in text:
            errors.append(f"{rel} missing v0.6.15 post-release hygiene checker")
        if rel != "scripts/release_check.sh" and "tests/test_v0615_post_release_hygiene.py" not in text:
            errors.append(f"{rel} missing v0.6.15 post-release hygiene tests")
...
def _payload(valid: bool, errors: list[str]) -> dict[str, Any]:
    return {
        "artifact_type": "v0615_post_release_hygiene_report",
        "schema_version": 1,
        "valid": valid,
        "current_public_release": CURRENT_PUBLIC,
        "source_version": SOURCE_VERSION,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(f"v0.6.15 post-release hygiene check {'PASS' if result['valid'] else 'FAIL'}")
