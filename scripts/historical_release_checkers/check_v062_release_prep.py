...
# ==============================================================================
# PROJECT: Atlas Agent
# FILE:    scripts/historical_release_checkers/check_v062_release_prep.py
# PURPOSE: Read-only v0.6.2 release prep checker (post-v0.6.2 state check).
...
def _check_version_bump() -> list[str]:
    ...
    for path in (PYPROJECT, INIT_PY):
        ...
        if POST_BUMP_VERSION_4 in text:
            found_post_bump = True
        if POST_BUMP_VERSION_5 in text:
            found_post_bump = True
    if not found_required and not found_post_bump:
        errors.append(f"Version {REQUIRED_VERSION}, {POST_BUMP_VERSION}, {POST_BUMP_VERSION_2}, {POST_BUMP_VERSION_3}, {POST_BUMP_VERSION_4}, or {POST_BUMP_VERSION_5} not found in pyproject.toml/__init__.py")
    return errors
